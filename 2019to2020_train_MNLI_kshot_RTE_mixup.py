...
def main():
    ...
    if args.do_train:
        ...
        target_sample_batch_size_16 = 16
        target_sample_batch_start_batch_16 = [x*target_sample_batch_size_16 for x in range(target_sample_size//target_sample_batch_size_16)]
        ...
        '''MixUp Phase'''
        for mixup_epoch in trange(int(args.mixup_epochs), desc="Mixup Epoch"):
            for target_sample_batch in target_samples_dataloader_batch_16:
                ...
                # for start_i in selected_source_batch_start_list:
                start_i = selected_target_sample_start_list[0]
                ids_single = target_sample_id_list[start_i:start_i+target_sample_batch_size_16]
                #target_samples_input_ids, target_samples_input_mask, target_samples_segment_ids, target_samples_label_ids
                single_target_sample_input_ids = target_samples_input_ids[ids_single].to(device)
                single_target_sample_input_mask = target_samples_input_mask[ids_single].to(device)
                single_target_sample_segment_ids = target_samples_segment_ids[ids_single].to(device)
                ...
                with torch.no_grad():
                    ...
                    target_sample_last3_reps_v2 = torch.cat([torch.mean(target_sample_logits_tuple_v2[1][-4], dim=1), torch.mean(target_sample_logits_tuple_v2[1][-3], dim=1), torch.mean(target_sample_logits_tuple_v2[1][-2], dim=1)], dim=1) #(batch, 1024*3)
