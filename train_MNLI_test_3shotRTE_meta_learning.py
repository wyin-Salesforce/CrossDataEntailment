...
class Encoder(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.mlp_2 = nn.Linear(config.hidden_size*3, 1, bias=False)
        # self.init_weights()
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, sample_size=None, class_size = None, labels=None, sample_labels=None, prior_samples_outputs=None, prior_samples_logits = None, few_shot_training=False, is_train = True, fetch_hidden_only=False):
        ...
        if is_train:
            ...
            '''??? add drop out here'''
            group_scores = torch.tanh(self.mlp_2(self.dropout(mlp_input+torch.tanh(self.mlp_1(self.dropout(mlp_input))))))#(batch*class_size, 1)
