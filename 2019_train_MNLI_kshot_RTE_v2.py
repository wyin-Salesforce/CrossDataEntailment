...
def main():
    ...
    if args.do_train:
        ...
        eval_features_entail_shot = eval_features_shot[:args.k_shot]
        eval_features_neutral_shot = eval_features_shot[args.k_shot:args.k_shot*2]
        eval_features_contra_shot = eval_features_shot[-args.k_shot:]

        eval_shot_dataloader_list = []
        for idd, train_features in enumerate([eval_features_entail_shot, eval_features_neutral_shot, eval_features_contra_shot,
            eval_features_entail_shot + eval_features_neutral_shot + eval_features_contra_shot]):
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(MNLI_dataloader, desc="Iteration")):
                ...
                '''each minibatch in MNLI has 10 examples'''
                ...
                '''sample from target side'''
                eval_shot_entail_batch = get_a_random_batch_from_dataloader(eval_shot_dataloader_list[0], 3)
                eval_shot_entail_batch_input_ids, eval_shot_entail_batch_input_mask, eval_shot_entail_batch_segment_ids, eval_shot_entail_batch_label_ids = tuple(t for t in eval_shot_entail_batch)

                eval_shot_neutral_batch = get_a_random_batch_from_dataloader(eval_shot_dataloader_list[1], 3)
                eval_shot_neutral_batch_input_ids, eval_shot_neutral_batch_input_mask, eval_shot_neutral_batch_segment_ids, eval_shot_neutral_batch_label_ids = tuple(t for t in eval_shot_neutral_batch)

                eval_shot_contra_batch = get_a_random_batch_from_dataloader(eval_shot_dataloader_list[2], 3)
                eval_shot_contra_batch_input_ids, eval_shot_contra_batch_input_mask, eval_shot_contra_batch_segment_ids, eval_shot_contra_batch_label_ids = tuple(t for t in eval_shot_contra_batch)

                eval_all_input_ids_shot = torch.cat([eval_shot_entail_batch_input_ids, eval_shot_neutral_batch_input_ids, eval_shot_contra_batch_input_ids], dim=0).to(device)
                eval_all_input_mask_shot = torch.cat([eval_shot_entail_batch_input_mask, eval_shot_neutral_batch_input_mask, eval_shot_contra_batch_input_mask], dim=0).to(device)
                ...
                model.train()
                loss_cross_domain, _ = model(torch.cat([eval_all_input_ids_shot,input_ids],dim=0), None, torch.cat([eval_all_input_mask_shot,input_mask], dim=0), sample_size=3, class_size =num_labels, labels=label_ids, sample_labels = torch.cuda.LongTensor([0,0,0,1,1,1,1,1,1]), prior_samples_outputs=None, is_train=True, loss_fct=loss_fct)
                ...
                model.train()
                loss_cross_sample, mnli_samples_outputs_i = model(torch.cat([sample_input_ids_i,eval_all_input_ids_shot],dim=0), None, torch.cat([sample_input_mask_i,eval_all_input_mask_shot], dim=0), sample_size=3, class_size =num_labels, labels=torch.cuda.LongTensor([0,0,0,1,1,1,1,1,1]), sample_labels = torch.cuda.LongTensor([0,0,0,1,1,1,2,2,2]), prior_samples_outputs=None, is_train=True, loss_fct=loss_fct)
                ...
                if iter_co %check_freq==0:
                    ...
                    for ff in range(2):
                        ...
                        for eval_shot_batch in eval_shot_dataloader_list[-1]:
                            ...
                            model.train()
                            few_loss = model(eval_shot_random_batch_input_ids, None, eval_shot_random_batch_input_mask, sample_size=3, class_size =num_labels, labels=None, sample_labels = eval_shot_random_batch_label_ids, prior_samples_outputs = None, few_shot_training=True, is_train=True, loss_fct=loss_fct)
                            few_loss.backward()
                            optimizer.step()
                            optimizer.zero_grad()
                    ...
                    for idd, dev_or_test_dataloader in enumerate([dev_dataloader, eval_dataloader]):
                        ...
                        for input_ids, input_mask, segment_ids, label_ids in dev_or_test_dataloader:
                            ...
                            '''
                            eval_all_input_ids_shot, eval_all_input_mask_shot
                            是上面随机得到的，预计直接用logits_LR的话不会用到
                            '''
