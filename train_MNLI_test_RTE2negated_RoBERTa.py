...
class RteProcessor(DataProcessor):
    ...
    def get_RTE2_negated_as_test(self, filename):
        ...
        for pair in root.findall('pair'):
            ...
            text_a = pair.find('t').text.strip()
            text_b = pair.find('h').text.strip()
            '''for RTE, we currently only choose randomly two labels in the set, in prediction we then decide the predicted labels'''
            label = 'contradiction'  if pair.get('contradiction') == 'YES' else 'neutral'
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
            line_co+=1
...
def main():
    ...
    if args.do_train:
        ...
        '''load dev set'''
        eval_examples = processor.get_RTE2_negated_as_test('/export/home/Dataset/RTE2_negated/RTE2_test_negated_contradiction.xml')
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if iter_co %20==0:
                    ...
                    for p in pred_indices:
                        pred_label_ids.append(2 if p == 2 else 1)
