...
class RteProcessor(DataProcessor):
    ...
    def get_SciTail_as_test(self, filename):
        readfile = codecs.open(filename, 'r', 'utf-8')
        line_co=0
        examples=[]
        for row in readfile:
            ...
            if len(line)==3:
                ...
                label = 'entailment'  if line[2].strip() == 'entails' else 'neutral'
                examples.append(
                    InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
                line_co+=1
...
def main():
    ...
    if args.do_train:
        ...
        '''load dev set'''
        eval_examples = processor.get_SciTail_as_test('/export/home/Dataset/SciTailV1/tsv_format/scitail_1.0_test.tsv')
